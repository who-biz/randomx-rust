...
struct Wrapper<T>(Arc<Mutex<Box<T>>>);
...
impl RxState {
    ...
    pub fn init_dataset(&mut self, threads_count: u8) -> Result<(), &str> {
        ...
        if dataset_ptr.is_null() {
            return Err("it's not possible to initialize a dataset");
        }
        ...
        for i in 0..threads_count {
            let cache = Wrapper(Arc::new(Mutex::new(Box::new(unsafe { *cache.cache }))));
            let dataset = Wrapper(Arc::new(Mutex::new(Box::new(unsafe { *dataset_ptr }))));
            let count = perth
